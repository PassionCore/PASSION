...
void PrepareShutdown()
{
    ...
    util::ThreadRename("passion-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), Passion_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), Passion_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Passion Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("passion-loadblk");
...
/** Sanity checks
 *  Ensure that Passion is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if(gArgs.IsArgSet("-stakeautolock")) {
        STAKE_AUTO_LOCK_UTXO = gArgs.GetBoolArg("-stakeautolock", true);
    }
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Passion process is using the data directory.
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // Passion: load previous sessions sporks if we have them.
