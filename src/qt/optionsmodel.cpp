...
void OptionsModel::setWalletDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (!settings.contains("bAutoLockStakeTierUTXO") || reset) {
        settings.setValue("bAutoLockStakeTierUTXO", STAKE_AUTO_LOCK_UTXO);
    }
    ...
    if (!settings.contains("subscriptionServiceIP") || reset) {
        settings.setValue("subscriptionServiceIP", "Please set address to the subscription server");
    }
...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PASSION);
    ...
    if (settings.contains("nAnonymizePassionAmount") || reset)
        gArgs.SoftSetArg("-anonymizepassionamount", settings.value("nAnonymizePassionAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in passion.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AutoLockStakeTierUTXO:
            return settings.value("bAutoLockStakeTierUTXO");
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case CombineUTXO:
            ...
            settings.setValue("fCombineUTXO", value.toBool());            
            break;
        case AutoLockStakeTierUTXO:
            settings.setValue("bAutoLockStakeTierUTXO",value.toBool());
            STAKE_AUTO_LOCK_UTXO = value.toBool();
            break;
