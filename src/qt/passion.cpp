...
#include "config/passion-config.h"
#endif
...
#include "qt/passion/splash.h"
#include "qt/passion/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("passion-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in passion.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in passion.qrc)
...
/** Class encapsulating Passion Core startup and shutdown.
...
/** Main Passion application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (PassionGUI)
...
{
    window = new PassionGUI(networkStyle, 0);
    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PassionGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PassionGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(PassionGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PassionGUI::DEFAULT_WALLET);
        ...
        // Passion: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PassionGUI::handlePaymentRequest);
        connect(window, &PassionGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Passion can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(passion_locale);
    Q_INIT_RESOURCE(passion);
    ...
    if (!Intro::pickDataDirectory())
        return 0;
    /// 6. Determine availability of data directory and parse passion.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Passion Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("Passion Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // passion: links repeatedly have their payment requests routed to this process:
