...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter Passion address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Passion: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::PASSION, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert passion:// to passion:
    //
    //    Cannot handle this later, because passion:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PASSION, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Passion (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Passion (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "passion.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Passion*.lnk
...
{
    return GetAutostartDir() / "passion.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a passion.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=Passion (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=Passion (regtest)\n";
        else
            optionFile << "Name=Passion\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the passion app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add passion app to startup item list
...
QString loadStyleSheet()
{
    ...
    QString tmp = "";
    fs::path pathAddr = GetDataDir(false);
    if(!pathAddr.empty()) {
        QString customCss = pathAddr.string().c_str() + tmp + "/themes/test.css";
        QFile qFile(customCss);
        if (qFile.open(QFile::ReadOnly)) {
            styleSheet = QLatin1String(qFile.readAll());
        } else {
            QFile qdefaultcss(cssName);
            if(qdefaultcss.open(QFile::ReadOnly)) {
                styleSheet = QLatin1String(qdefaultcss.readAll());
            }
        }
    }
    return styleSheet;
