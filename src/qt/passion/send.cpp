...
#include "qt/passion/send.h"
#include "qt/passion/forms/ui_send.h"
#include "qt/passion/addnewcontactdialog.h"
#include "qt/passion/qtutils.h"
#include "qt/passion/sendchangeaddressdialog.h"
#include "qt/passion/optionbutton.h"
#include "qt/passion/sendconfirmdialog.h"
#include "qt/passion/guitransactionsutils.h"
#include "qt/passion/loadingdialog.h"
...
SendWidget::SendWidget(PassionGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-passion");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onPASSIONSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onPASSIONSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent PASSIONs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield PASSIONs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent PASSIONs to shield."));
    }
}
...
void SendWidget::onPASSIONSelected(bool _isTransparent)
{
