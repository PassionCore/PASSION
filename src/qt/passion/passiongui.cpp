...
#include "qt/passion/defaultdialog.h"
...
const QString PassionGUI::DEFAULT_WALLET = "~Default";

PassionGUI::PassionGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void PassionGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &PassionGUI::toggleHidden);
 ...
 */
void PassionGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &PassionGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &PassionGUI::changeTheme);
    ...
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &PassionGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &PassionGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &PassionGUI::execDialog);
}


void PassionGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Passion Core client") + " " + networkStyle->getTitleAddText();
...
void PassionGUI::handleRestart(QStringList args)
{
...
void PassionGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        subscriptionWidget->setClientModel(clientModel);

        // Receive and report messages from client model
        connect(clientModel, &ClientModel::message, this, &PassionGUI::message);
...
void PassionGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &PassionGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &PassionGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void PassionGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void PassionGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void PassionGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &PassionGUI::hide);
                e->ignore();
            }
        }
    }
...
void PassionGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void PassionGUI::messageInfo(const QString& text)
{
...
void PassionGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
    QString strTitle =  tr("Passion Core"); // default title
...
bool PassionGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Passion Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void PassionGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void PassionGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PassionGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void PassionGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void PassionGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void PassionGUI::goToSettings(){
    showTop(settingsWidget);
}

void PassionGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void PassionGUI::goToReceive()
{
    showTop(receiveWidget);
}

void PassionGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void PassionGUI::showTop(QWidget* view)
{
...
void PassionGUI::changeTheme(bool isLightTheme)
{
...
void PassionGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool PassionGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void PassionGUI::showHide(bool show)
{
...
int PassionGUI::getNavWidth()
{
    return this->navMenu->width();
}

void PassionGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
bool PassionGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(dashboard, &DashboardWidget::message, this, &PassionGUI::message);
    connect(walletModel, &WalletModel::message, this, &PassionGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &PassionGUI::message);
    connect(topBar, &TopBar::message, this, &PassionGUI::message);
    connect(sendWidget, &SendWidget::message,this, &PassionGUI::message);
...
bool PassionGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void PassionGUI::removeAllWallets()
{
    // Single wallet supported.
}

void PassionGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(PassionGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void PassionGUI::subscribeToCoreSignals()
{
...
void PassionGUI::unsubscribeFromCoreSignals()
{
