...
#include "qt/passion/forms/ui_snackbar.h"
#include "qt/passion/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(PassionGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &PassionGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
