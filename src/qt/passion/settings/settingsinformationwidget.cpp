...
#include "qt/passion/settings/settingsinformationwidget.h"
#include "qt/passion/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(PassionGUI* _window,QWidget *parent) :
    ...
    ui(new Ui::SettingsInformationWidget)
{
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open passion.conf with default application"));
    });
...
void SettingsInformationWidget::run(int type)
{
    if (type == REQUEST_UPDATE_COUNTS) {
        QMetaObject::invokeMethod(this, "setNumBlocks",Qt::QueuedConnection, Q_ARG(int, clientModel->getLastBlockProcessedHeight()));
    }
}
