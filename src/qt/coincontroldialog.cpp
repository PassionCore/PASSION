...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select PASSION Outputs to Spend" :
            "Select Shielded PASSION to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::PASSION;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show Passion address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
