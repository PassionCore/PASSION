        ...
        QList<TransactionRecord>& parts)
{
    ...
    if (isminetype mine = wallet->IsMine(wtx.tx->vout[1])) {
        ...
        if(wtx.tx->vout.size() >=2) {
            int n = (int) wtx.tx->vout.size();
            for(int i = 2; i < n; i++) {
                isminetype mine;              
                if (ExtractDestination(wtx.tx->vout[i].scriptPubKey, dest) && (mine = IsMine(*wallet, dest))){
                    TransactionRecord subw(hash, wtx.GetTxTime(), wtx.tx->GetTotalSize());
                    subw.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;                    
                    subw.address = EncodeDestination(dest);
                    subw.credit = wtx.tx->vout[i].nValue;
                    if(subw.credit == CMultiTier::getInstance()->GetMultiTierValue(blockvalue,1)) {
                        subw.type = TransactionRecord::StakeRewardTier1;
                    } else if(subw.credit == CMultiTier::getInstance()->GetMultiTierValue(blockvalue,2)) {
                        subw.type = TransactionRecord::StakeRewardTier2;
                    } else if(subw.credit == CMultiTier::getInstance()->GetMultiTierValue(blockvalue,3)) {
                        subw.type = TransactionRecord::StakeRewardTier3;
                    } else if(subw.credit == CMultiTier::getInstance()->GetMultiTierValue(blockvalue,4)) {
                        subw.type = TransactionRecord::StakeRewardTier4;
                    }
                    parts.append(subw);
                }
            }
            ...
            if (ExtractDestination(wtx.tx->vout[nIndexMN].scriptPubKey, dest) && mine ) {
                TransactionRecord subw(hash, wtx.GetTxTime(), wtx.tx->GetTotalSize());
                subw.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned PASSIONs, op_return could be for a proposal/budget fee or another sort of data stored there.
...
void TransactionRecord::updateStatus(const CWalletTx& wtx, int chainHeight)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeRewardTier1 ||
            type == TransactionRecord::StakeRewardTier2 ||
            type == TransactionRecord::StakeRewardTier3 ||
            type == TransactionRecord::StakeRewardTier4 ||
