...
#if defined(NDEBUG)
#error "Passion cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
    ...
    if (!CheckTransaction(tx, state, fColdStakingActive, fSaplingActive))
...
CAmount GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
     if (nHeight <= 100000 && nHeight > 0) {
        nSubsidy = 15 * COIN; // Proof of Work
    } else if (nHeight <= POSPHASE::PHASE0 && nHeight > 100000) {
        nSubsidy = 12 * COIN; // Phase 1
    } else if (nHeight <= POSPHASE::PHASE1 && nHeight > POSPHASE::PHASE0) {
        nSubsidy = 10 * COIN; // Phase 2
    }  else if (nHeight <= POSPHASE::PHASE2 && nHeight > POSPHASE::PHASE1) {
        nSubsidy = 8 * COIN; // Phase 3
    }  else if (nHeight <= POSPHASE::PHASE3 && nHeight > POSPHASE::PHASE2) {
        nSubsidy = 7 * COIN; // Phase 4
    }  else if (nHeight <= POSPHASE::PHASE4 && nHeight > POSPHASE::PHASE3) {
        nSubsidy = 8 * COIN; // Phase 5
    }  else if (nHeight <= POSPHASE::PHASE5 && nHeight > POSPHASE::PHASE4) {
        nSubsidy = 9 * COIN; // Phase 6
    }  else if (nHeight <= POSPHASE::PHASE6 && nHeight > POSPHASE::PHASE5) {
        nSubsidy = 10 * COIN; // Phase 7
    }  else if (nHeight <= POSPHASE::PHASE7 && nHeight > POSPHASE::PHASE6) {
        nSubsidy = 9 * COIN; // Phase 8
    }  else if (nHeight <= POSPHASE::PHASE8 && nHeight > POSPHASE::PHASE7) {
        nSubsidy = 8 * COIN; // Phase 9
    }  else if (nHeight <= POSPHASE::PHASE9 && nHeight > POSPHASE::PHASE8) {
        nSubsidy = 7 * COIN; // Phase 10
    }  else if (nHeight <= POSPHASE::PHASE10 && nHeight > POSPHASE::PHASE9) {
        nSubsidy = 6 * COIN; // Phase 11
    }  else if (nHeight <= POSPHASE::PHASE11 && nHeight > POSPHASE::PHASE10) {
        nSubsidy = 5 * COIN; // Phase 12
    }  else if (nHeight <= POSPHASE::PHASE12 && nHeight > POSPHASE::PHASE11) {
        nSubsidy = 4 * COIN; // Phase 13
    }  else if (nHeight <= POSPHASE::PHASE13 && nHeight > POSPHASE::PHASE12) {
        nSubsidy = 3 * COIN; // Phase 14
    }  else if (nHeight <= POSPHASE::PHASE14 && nHeight > POSPHASE::PHASE13) {
...
bool IsInitialBlockDownload()
{
    ...
    if (fImporting || fReindex || fVerifyingBlocks || chainHeight < Checkpoints::GetTotalBlocksEstimate())
        return true;
    bool state = (chainHeight < pindexBestHeader->nHeight - 24 * 6 || pindexBestHeader->GetBlockTime() < GetTime() - nMaxTipAge);
    if (!state)
...
DisconnectResult DisconnectBlock(CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        // not coinbases spend because they dont have traditional inputs
        if (tx.IsCoinBase())
            continue;
...
{
    util::ThreadRename("passion-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            LogPrintf("%s: Validate MultiTier payment checks sync\n", __func__);
        }
    }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Passion Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nPassionBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nPassionBadBlockBits)) {
            // accept Passion block minted with incorrect proof of work threshold
            return true;
        }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if ((block.nVersion < 3 && nHeight >= 1) || (block.nVersion < 8 && consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V5_0)))
    {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        const bool hasPASSIONInputs = !pivInputs.empty();
...
static bool RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if(!CMultiTier::getInstance()->CheckTransactionMultiTier(pindex->nHeight, block, GetBlockValue(pindex->nHeight), pindex, inputs)) {
            ...
            //std::cout << "RollBackForwardd: Multi-Tier-Staking block was NOT valid" << std::endl;
