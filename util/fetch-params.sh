...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PassionParams"
    else
        PARAMS_DIR="$HOME/.passion-params"
    fi
fi
...
Failed to fetch the Passion zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Passion - fetch-params.sh

This script will fetch the Passion zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Passion zkSNARK parameters. Note that it is
